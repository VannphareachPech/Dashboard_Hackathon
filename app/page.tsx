...
function Section({ title, id, children }: { title: string; id?: string; children: React.ReactNode }) {
  return (
    <section id={id} className="h-full flex flex-col space-y-2.5 scroll-mt-20">
...
export default async function DashboardPage() {
  ...
  const { cycle, generatedDate, narrativeSummary, summary, areaScores, trends, recommendations, actions, roleSplit, responseCounts, responseMix, prevCycle, focusSuggestion } = data;

  // Use the current-cycle count from responseCounts (per-cycle accuracy) rather
  // than summary.totalResponses which is the all-time total across all pulses.
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 py-6">
        <main>
          ...
          <section id="overview" className="space-y-4 scroll-mt-20">
            ...
            {/* Overall Team Sentiment + Leadership Readout */}
          ...
          {/* ════════════════════════════════════════════════════════
               GROUP 4 — Themes + Leadership Actions
               Comments & Themes → Next Steps
          ════════════════════════════════════════════════════════ */}
          <GroupDivider />
          <div className="mt-5 space-y-5">
